...
DEFAULT_PARAMS = {
    ...
    "post_recovery_steps": 36,
    "post_recovery_speed_cap": 3.4,
    "post_recovery_steer_bias": 0.22,
    "endurance_step_threshold": 145000,
    "endurance_straight_speed": 6.9,
    "endurance_corner_speed_scale": 3.1,
    "endurance_corner_speed_cap": 2.85,
    "endurance_front_speed_gain": 4.15,
}
...
class Driver:
    def __init__(self):
        ...
        self.post_recovery_steps_remaining = 0
        self.step_count = 0
    ...
    def corner_speed(self, steering_angle):
        ...
        scale = self.params["corner_speed_scale"]
        cap = self.params["corner_speed_cap"]
        if self.in_endurance_mode():
            scale = self.params["endurance_corner_speed_scale"]
            cap = self.params["endurance_corner_speed_cap"]
        scaled = scale * normalized ** self.params["corner_power"]
    ...
    def in_endurance_mode(self):
        return self.step_count >= self.params["endurance_step_threshold"]
    ...
    def maybe_recover(self, processed, front_clear, front_tight):
        ...
        if self.recovery_steps_remaining > 0:
            ...
            if front_clear > 0.55 and front_tight > self.params["stall_front_threshold"]:
                self.recovery_steps_remaining = 0
                self.recovery_cycles = 0
                self.post_recovery_steps_remaining = int(self.params["post_recovery_steps"])
                return None
    ...
    def process_lidar(self, ranges):
        ...
        straight_speed = self.params["straight_speed"]
        front_speed_gain = self.params["front_speed_gain"]
        if self.in_endurance_mode():
            straight_speed = self.params["endurance_straight_speed"]
            front_speed_gain = self.params["endurance_front_speed_gain"]
        ...
        speed = min(speed, max(self.params["front_speed_floor"], front_speed_gain * front_clear))
        speed *= self.speed_scale()

        recovery = self.maybe_recover(processed, front_clear, front_tight)
        if recovery is not None:
            speed, steering_angle = recovery
        elif self.post_recovery_steps_remaining > 0:
            self.post_recovery_steps_remaining -= 1
            speed = min(speed, self.params["post_recovery_speed_cap"])
            if abs(steering_angle) < self.params["post_recovery_steer_bias"]:
                steering_angle = self.recovery_direction * self.params["post_recovery_steer_bias"]
