...
DEFAULT_PARAMS = {
    ...
    "straight_speed": 6.6,
    "boosted_straight_speed": 6.9,
    "straight_angle_threshold": 0.1,
    "straight_distance_threshold": 0.5,
    "corner_speed_scale": 2.95,
    "corner_speed_cap": 2.7,
    "corner_power": 1.0,
    "front_speed_gain": 4.1,
    "front_speed_floor": 1.4,
    "rear_ignore_fraction": 0.125,
    "bucket_count": 20,
    "boosted_buckets": [],
    "bucket_speed_overrides": {11: 6.9, 13: 5.9, 14: 5.5, 15: 5.3, 18: 5.2, 19: 4.8},
    "recovery_enable_after_completion": 100.0,
    "stall_speed_threshold": 0.12,
    "stall_front_threshold": 0.22,
    "stall_trigger_steps": 24,
    "stuck_speed_threshold": 0.55,
    "progress_timeout_seconds": 0.9,
    "regression_margin": 1.0,
    "regression_trigger_steps": 4,
    "heading_recovery_trigger": 2.2,
    "reverse_steps": 12,
    "recovery_steps": 26,
    "reverse_speed": -1.5,
    "reverse_steer": 1.1,
    ...
    "speed_scale_drop_per_recovery": 0.03,
    "speed_scale_floor": 0.84,
}
...
class Driver:
    def __init__(self):
        ...
        self.recovery_baseline_completion = -1e9
    ...
    def update_progress(self, state, front_clear, actual_speed):
        ...
        if absolute_completion > self.best_absolute_completion:
            ...
            self.last_progress_time = float(state.time)
    ...
    def start_recovery(self, completion, heading_error):
        ...
        self.recovery_baseline_completion = self.best_absolute_completion
        self.stall_steps = 0
        self.regression_steps = 0
        self.total_recoveries += 1

    def maybe_recover(self, completion, heading_error, front_clear, actual_speed, now):
        ...
        if self.recovery_steps_remaining > 0:
            ...
            progressed = self.best_absolute_completion > self.recovery_baseline_completion + 0.6
            if (abs(heading_error) < 0.4 and actual_speed > 0.8) or progressed:
                self.recovery_steps_remaining = 0
                self.recovery_cycles = 0
                return None
            return self.params["recovery_speed"], self.recovery_direction * self.params["recovery_steer"]

        if self.recovery_cycles > 0 and self.best_absolute_completion <= self.recovery_baseline_completion + 0.2:
            ...
            self.recovery_steps_remaining = int(self.params["recovery_steps"])
            self.recovery_baseline_completion = self.best_absolute_completion
            return self.maybe_recover(completion, heading_error, front_clear, actual_speed, now)
        ...
        should_recover = (
            ...
            or (now - self.last_progress_time > self.params["progress_timeout_seconds"] and actual_speed < self.params["stuck_speed_threshold"])
    ...
    def process_lidar(self, ranges, state):
        ...
        recovery = self.maybe_recover(completion, heading_error, front_clear, actual_speed, float(state.time))
